library(readr)
sc_markers_6oct25 <- read_csv("/filepath/sc_markers_6oct25.csv")
library(Biobase)
sc_markers = sc_markers_6oct25
e_this <- readRDS("/filepath/e_this_7oct25.rds")
...
View(markers_df)
write.csv(markers_df, file = "/filepath/markers_7oct25.csv", row.names = FALSE)
all_genes <- unlist(markers)
unique_genes <- unique(all_genes)
print(unique_genes)
write.csv(unique_genes, file = "/filepath/unique_genes_7oct25.csv", row.names = FALSE)
